...
function Lists() {
  ...
  const toggleVisibility = async (id, isVisible) => {
    try {
      // Backend endpoint should handle this flag (update visibility)
      await axios.post(`${serverUrl}/api/product/visibility/${id}`, { visible: isVisible }, { withCredentials: true })
      // Update local state
      setList(prev =>
        prev.map(item =>
          item._id === id ? { ...item, visible: isVisible } : item
        )
      )
    } catch (error) {
      console.log(error)
                ...
                >
                  ...
                  <div className='flex items-center gap-2'>
                    <input
                      ...
                      onChange={(e) => toggleVisibility(item._id, e.target.checked)}
                      className='w-[20px] h-[20px] cursor-pointer accent-[#2f97f1]'
                    />
                    <label className='text-[14px] md:text-[16px]'>Show</label>
                  </div>
